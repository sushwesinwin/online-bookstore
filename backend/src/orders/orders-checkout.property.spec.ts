            ...
            await fc.assert(
                fc.asyncProperty(
                    ...
                    async testData => {
                        ...
                        expect(Math.abs(capturedTotalAmount! - expectedTotal)).toBeLessThan(0.01);
                    },
                ),
                { numRuns: 100 },
            );
            ...
            await fc.assert(
                fc.asyncProperty(
                    ...
                    async testData => {
                        ...
                        const inventoryUpdates: Array<{ bookId: string; decrement: number }> = [];
                        ...
                        for (const item of testData.items) {
                            const update = inventoryUpdates.find(u => u.bookId === item.bookId);
                ...
                fc.asyncProperty(
                    ...
                    async testData => {
                        ...
                        for (let i = 0; i < testData.items.length; i++) {
                            ...
                            expect(Math.abs(capturedPrice - item.currentPrice)).toBeLessThan(0.01);
                        }
                    },
                ),
                { numRuns: 100 },
