...
export class PaymentService {
     ...
     */
    async createCheckoutSessionFromCart(userId: string): Promise<Stripe.Checkout.Session> {
        try {
            // Get user's cart
    ...
    async handleCheckoutSessionCompleted(session: Stripe.Checkout.Session): Promise<void> {
        try {
            ...
            if (!cart.items || cart.items.length === 0) {
                this.logger.error(`Cart is empty for user ${userId} during checkout completion`);
                return;
            }
