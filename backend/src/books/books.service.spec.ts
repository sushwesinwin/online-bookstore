    ...
    describe('Property-Based Tests', () => {
        ...
        it('should maintain book data integrity for all valid inputs', async () => {
            await fc.assert(
                fc.asyncProperty(
                    fc.record({
                        isbn: fc.string({ minLength: 10, maxLength: 13 }).map(s => s.replace(/[^0-9X]/g, '0')),
                        title: fc.string({ minLength: 1, maxLength: 200 }),
                        author: fc.string({ minLength: 1, maxLength: 100 }),
                        description: fc.option(fc.string({ maxLength: 1000 }), { nil: undefined }),
                        price: fc.double({ min: 0.01, max: 9999.99, noNaN: true }).map(p => Math.round(p * 100) / 100),
            ...
            await fc.assert(
                fc.asyncProperty(
                    ...
                    async (bookId, initialInventory) => {
                        ...
                        const result = await service.updateInventory(bookId, initialInventory);
            ...
            await fc.assert(
                fc.asyncProperty(
                    ...
                    async (searchTerm, page, limit) => {
                        ...
                        const mockBooks = Array.from({ length: Math.min(limit, 10) }, (_, i) => ({
                            id: `book-${i}`,
                            isbn: `123456789${i}`,
                            title: i % 3 === 0 ? `${searchTerm} Book` : `Book ${i}`,
                            author: i % 3 === 1 ? `${searchTerm} Author` : `Author ${i}`,
                            description: i % 3 === 2 ? `Description with ${searchTerm}` : `Description ${i}`,
            ...
            await fc.assert(
                fc.asyncProperty(
                    fc.record({
                        ...
                        minPrice: fc.option(fc.double({ min: 0.01, max: 50, noNaN: true }).map(p => Math.round(p * 100) / 100), { nil: undefined }),
                        maxPrice: fc.option(fc.double({ min: 50, max: 100, noNaN: true }).map(p => Math.round(p * 100) / 100), { nil: undefined }),
            ...
            await fc.assert(
                fc.asyncProperty(
                    ...
                    async (bookData) => {
                        ...
                        await expect(service.create(bookData as CreateBookDto))
                            .rejects
            ...
            await fc.assert(
                fc.asyncProperty(
                    ...
                    fc.record({
                        ...
                        price: fc.option(fc.double({ min: 0.01, max: 9999.99, noNaN: true }).map(p => Math.round(p * 100) / 100), { nil: undefined }),
                        inventory: fc.option(fc.integer({ min: 0, max: 10000 }), { nil: undefined }),
                    }),
                    async (bookId, updateData) => {
                        const mockUpdatedBook = {
                            ...
                            inventory: updateData.inventory !== undefined ? updateData.inventory : 10,
                            category: 'Fiction',
                            imageUrl: null,
                            createdAt: new Date(),
                            updatedAt: new Date(),
                        ...
                        const result = await service.update(bookId, updateData as UpdateBookDto);
    ...
    describe('Unit Tests', () => {
        describe('create', () => {
            ...
            it('should throw ConflictException for duplicate ISBN', async () => {
                ...
                await expect(service.create(bookData)).rejects.toThrow(ConflictException);
                await expect(service.create(bookData)).rejects.toThrow('Book with this ISBN already exists');
            });
        ...
        describe('findOne', () => {
            ...
            it('should throw NotFoundException when book not found', async () => {
                ...
                await expect(service.findOne('non-existent-id')).rejects.toThrow('Book not found');
            });
        });
        ...
        describe('update', () => {
            ...
            it('should throw ConflictException when updating to duplicate ISBN', async () => {
                ...
                await expect(service.update('test-id', updateData)).rejects.toThrow(ConflictException);
            });
        });
