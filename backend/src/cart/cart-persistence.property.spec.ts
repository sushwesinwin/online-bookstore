            ...
            await fc.assert(
                fc.asyncProperty(
                    fc.record({
                        ...
                        cartItems: fc.array(
                            fc.record({
                                ...
                                bookPrice: fc.double({ min: 0.01, max: 999.99, noNaN: true })
                    ...
                    async (testData) => {
                        ...
                        expect(Math.abs(retrievedCart.total - expectedTotal)).toBeLessThan(0.01);
            ...
            await fc.assert(
                fc.asyncProperty(
                    fc.record({
                        ...
                        bookPrice: fc.double({ min: 0.01, max: 99.99, noNaN: true })
                            .map(p => Math.round(p * 100) / 100),
                    }),
                    async (testData) => {
                        ...
                        mockPrismaService.cartItem.findMany.mockResolvedValueOnce(user1CartItems);
                        const user1Cart = await service.getCart(testData.user1Id);

                        // Retrieve user 2's cart
                        mockPrismaService.cartItem.findMany.mockResolvedValueOnce(user2CartItems);
                        ...
                        expect(user1Cart.total).toBe(testData.bookPrice * testData.user1Quantity);
                        expect(user2Cart.total).toBe(testData.bookPrice * testData.user2Quantity);
                    }
                ),
                { numRuns: 100 }
            );
            ...
            await fc.assert(
                fc.asyncProperty(
                    ...
                    async (testData) => {
                        ...
                        const unavailableCartItems = testData.unavailableItems.map(item => ({
                            id: item.id,
                            userId: testData.userId,
                            bookId: item.bookId,
                            quantity: item.cartQuantity,
                        ...
                        const allCartItems = [...availableCartItems, ...unavailableCartItems];
                        ...
                        expect(validation.valid).toBe(false);
                        expect(validation.issues.length).toBe(testData.unavailableItems.length);
            ...
            await fc.assert(
                fc.asyncProperty(
                    ...
                    async (testData) => {
                        ...
                        for (const item of testData.cartItems) {
                            const hasIssue = validation.issues.some(issue =>
                                issue.includes(item.bookTitle) && issue.includes('Available: 0')
                            );
                            expect(hasIssue).toBe(true);
                        }
                    }
            ...
            await fc.assert(
                fc.asyncProperty(
                    ...
                    async (testData) => {
                        ...
                        expect(session1Cart.items.length).toBe(testData.initialCartItems.length);
                        expect(session2Cart.items.length).toBe(testData.initialCartItems.length);
                        expect(session3Cart.items.length).toBe(testData.initialCartItems.length);
                        ...
                        expect(mockPrismaService.cartItem.findMany).toHaveBeenCalledTimes(3);
                    }
                ),
                { numRuns: 100 }
            );
