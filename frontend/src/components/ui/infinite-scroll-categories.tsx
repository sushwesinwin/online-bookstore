        ...
        <div className="relative overflow-hidden py-8">
            ...
            <div className="flex animate-infinite-scroll">
                ...
                {categories.map((category, index) => (
                    <Link key={`first-${index}`} href={`/books?category=${category.name}`}>
                        <div className="group relative bg-white rounded-xl p-6 mx-3 text-center hover:shadow-xl transition-all duration-300 cursor-pointer border border-[#E4E9E8] min-w-[140px]">
                            <div className={`absolute inset-0 bg-linear-to-br ${category.color} opacity-0 group-hover:opacity-10 rounded-xl transition-opacity`} />
                            <div className="text-4xl mb-3">{category.emoji}</div>
                            <div className="font-semibold text-[#101313] whitespace-nowrap">{category.name}</div>
                        </div>
                    </Link>
                ))}
                ...
                {categories.map((category, index) => (
                    <Link key={`second-${index}`} href={`/books?category=${category.name}`}>
                        <div className="group relative bg-white rounded-xl p-6 mx-3 text-center hover:shadow-xl transition-all duration-300 cursor-pointer border border-[#E4E9E8] min-w-[140px]">
                            <div className={`absolute inset-0 bg-linear-to-br ${category.color} opacity-0 group-hover:opacity-10 rounded-xl transition-opacity`} />
                            <div className="text-4xl mb-3">{category.emoji}</div>
                            <div className="font-semibold text-[#101313] whitespace-nowrap">{category.name}</div>
