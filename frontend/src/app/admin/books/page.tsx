...
import { useBooks, useCategories, useCreateBook, useUpdateBook, useDeleteBook } from '@/lib/hooks/use-books';
    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h1 className="text-3xl font-bold tracking-tight text-[#101313]">Books Catalog</h1>
                    <p className="mt-1 text-[#848785]">Manage your bookstore library and inventory.</p>
                </div>
                <Button
                    ...
                    <table className="w-full text-left">
                        <thead>
                            <tr className="border-b border-[#E4E9E8] bg-[#F8FAFB]">
                                <th className="px-6 py-5 text-xs font-semibold uppercase tracking-wider text-[#848785]">Book</th>
                                <th className="px-6 py-5 text-xs font-semibold uppercase tracking-wider text-[#848785]">Category</th>
                                <th className="px-6 py-5 text-xs font-semibold uppercase tracking-wider text-[#848785]">Inventory</th>
                                <th className="px-6 py-5 text-xs font-semibold uppercase tracking-wider text-[#848785]">Price</th>
                                <th className="px-4 py-5 text-right text-xs font-semibold uppercase tracking-wider text-[#848785]">Actions</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-[#E4E9E8]">
                            {isLoading ? (
                                ...
                                <tr key={book.id} className="group transition-colors hover:bg-[#F8FAFB]/50">
                                    <td className="px-6 py-4">
                                        <div className="flex items-center space-x-4">
                                            ...
                                            <div>
                                                <div className="font-bold text-[#101313] line-clamp-1">{book.title}</div>
                                                <div className="text-xs text-[#848785]">{book.author}</div>
                                                <div className="mt-1 text-[10px] text-[#A6AAA9]">{book.isbn}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4">
                                        <Badge variant="outline" className="border-[#E4E9E8] bg-[#F8FAFB] px-3 font-medium text-[#101313]">
                                            {book.category}
                                        </Badge>
                                    </td>
                                    <td className="px-6 py-4">
                                        <div className="flex items-center">
                                            <div
                                                className={`mr-2 h-2 w-2 rounded-full ${book.inventory > 10 ? 'bg-[#17BD8D]' :
                                                    book.inventory > 0 ? 'bg-[#F9B959]' : 'bg-[#FF4E3E]'
                                                    }`}
                                            />
                                            <span className="font-semibold text-[#101313]">{book.inventory}</span>
                                            <span className="ml-1 text-xs text-[#848785]">in stock</span>
                                        </div>
                                    </td>
                ...
                {data && data.meta.totalPages > 1 && (
                    <div className="flex items-center justify-between border-t border-[#E4E9E8] bg-[#F8FAFB] px-6 py-4">
                        <p className="text-sm text-[#848785]">
                            Showing <span className="font-semibold text-[#101313]">{(page - 1) * 10 + 1}</span> to <span className="font-semibold text-[#101313]">{Math.min(page * 10, data.meta.total)}</span> of <span className="font-semibold text-[#101313]">{data.meta.total}</span> books
                        </p>
            ...
            >
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="grid grid-cols-2 gap-6">
                        <div className="space-y-2">
                            <label className="text-sm font-bold text-[#101313]">Book Title</label>
                            <Input
                                required
                        ...
                        <div className="space-y-2">
                            <label className="text-sm font-bold text-[#101313]">Author Name</label>
                            <Input
                                required
                        ...
                        <div className="space-y-2">
                            <label className="text-sm font-bold text-[#101313]">Category</label>
                            <select
                                required
                        ...
                        <div className="space-y-2">
                            <label className="text-sm font-bold text-[#101313]">ISBN Number</label>
                            <Input
                                required
                                value={formData.isbn}
                        ...
                        <div className="space-y-2">
                            <label className="text-sm font-bold text-[#101313]">Price ($)</label>
                            <Input
                                required
                                type="number"
                                step="0.01"
                        ...
                        <div className="space-y-2">
                            <label className="text-sm font-bold text-[#101313]">Inventory (Stock)</label>
                            <Input
                                required
                                type="number"
                    ...
                    <div className="space-y-2">
                        <label className="text-sm font-bold text-[#101313]">Image URL</label>
                        <Input
                            required
                    ...
                    <div className="space-y-2">
                        <label className="text-sm font-bold text-[#101313]">Description</label>
                        <textarea
                    ...
                    <div className="flex justify-end space-x-3 pt-4">
                        ...
                        >
                            {(createBook.isPending || updateBook.isPending) && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
