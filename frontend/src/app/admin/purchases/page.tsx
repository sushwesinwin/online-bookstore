...
import { Search, Loader2, Plus, Minus, Trash2, Check, ShoppingCart, ArrowRight } from 'lucide-react';
...
export default function NewPurchasePage() {
    ...
    const { data: usersData, isLoading: isLoadingUsers } = useQuery({
        ...
        queryFn: () => adminApi.getUsers({ search: debouncedUserSearch, limit: 10 }),
        enabled: step === 1,
    });

    const { data: booksData, isLoading: isLoadingBooks } = useQuery({
        queryKey: ['books', debouncedBookSearch],
        queryFn: () => booksApi.getBooks({ search: debouncedBookSearch, limit: 12 }),
        enabled: step === 2,
    });
    ...
    const totalAmount = selectedBooks.reduce((acc, curr) => acc + curr.price * curr.quantity, 0);
        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    ...
                    <p className="mt-2 text-[#848785]">Create a manual purchase order for a customer.</p>
                </div>
            </div>
            ...
            <div className="bg-white rounded-3xl border border-[#E4E9E8] shadow-sm overflow-hidden">
                ...
                <div className="flex bg-[#F8FAFB] border-b border-[#E4E9E8]">
                    <button
                        onClick={() => setStep(1)}
                        className={`flex-1 py-4 text-sm font-bold transition-colors ${step === 1 ? 'text-[#0B7C6B] border-b-2 border-[#0B7C6B]' : 'text-[#848785] hover:bg-[#F3F5F5]'
                            }`}
                    >
                        1. Select Customer
                    </button>
                    <button
                        ...
                        className={`flex-1 py-4 text-sm font-bold transition-colors ${step === 2 ? 'text-[#0B7C6B] border-b-2 border-[#0B7C6B]' : 'text-[#848785] hover:bg-[#F3F5F5]'
                                ...
                                {isLoadingUsers ? (
                                    ...
                                    usersData?.data?.map((user) => (
                                        <div
                                            ...
                                            className={`p-4 flex items-center justify-between cursor-pointer transition-all ${selectedUserId === user.id
                        ...
                        <div className="grid lg:grid-cols-3 gap-8">
                                    ...
                                    {isLoadingBooks ? (
                                        ...
                                        booksData?.data?.map((book) => {
                                            ...
                                            return (
                                                <div
                                                    key={book.id}
                                                    className={`p-4 rounded-2xl border flex flex-col justify-between transition-all ${isSelected ? 'border-[#0B7C6B] bg-[#E4FFFB]' : 'border-[#E4E9E8] hover:border-[#0B7C6B]/30 hover:shadow-md bg-white'
                                                        }`}
                                                >
                                                    <div className="flex items-start gap-3 mb-4">
                                                        {book.imageUrl ? (
                                                            <img src={book.imageUrl} alt={book.title} className="w-16 h-20 object-cover rounded shadow-sm" />
                                                        ) : (
                                                            <div className="w-16 h-20 bg-[#F3F5F5] rounded flex items-center justify-center flex-shrink-0">
                                                                <span className="text-[#A6AAA9] text-xs">No Cover</span>
                                                            </div>
                                                        )}
                                                        <div>
                                                            <p className="font-bold text-[#101313] text-sm line-clamp-2">{book.title}</p>
                                                            <p className="text-[#848785] text-xs mt-1">{book.author}</p>
                                                            <p className="font-bold text-[#0B7C6B] mt-2">${book.price}</p>
                                                        </div>
                                                    </div>
                                                    <Button
                                                        variant={isSelected ? "outline" : "default"}
                                                        className={`w-full ${isSelected ? "border-[#0B7C6B] text-[#0B7C6B]" : "bg-[#0B7C6B] hover:bg-[#096B5B] text-white"}`}
                                                        onClick={() => isSelected ? removeBook(book.id) : addBook(book)}
                                                    >
                            ...
                            <div className="flex flex-col h-[500px] border border-[#E4E9E8] rounded-2xl overflow-hidden bg-white shadow-sm">
                                <div className="bg-[#F8FAFB] px-6 py-4 border-b border-[#E4E9E8]">
                                    <h3 className="font-black text-lg text-[#101313]">Order Summary</h3>
                                    ...
                                    {selectedBooks.length === 0 ? (
                                        ...
                                        <div className="divide-y divide-[#E4E9E8]">
                                            {selectedBooks.map((book) => (
                                                <div key={book.id} className="p-4 hover:bg-[#F8FAFB] rounded-xl transition-colors mb-1">
                                                    ...
                                                    <div className="flex items-center justify-between">
                                                        <div className="flex items-center gap-1 bg-[#F3F5F5] rounded-lg p-1">
                                                            ...
                                                            <span className="text-sm font-bold w-6 text-center">{book.quantity}</span>
                                                            <button
                                                                onClick={() => updateQuantity(book.id, 1)}
                                                                className="p-1.5 hover:bg-white rounded-md transition-colors"
                                                            >
                                                        ...
                                                        <p className="font-bold text-[#101313]">${(book.price * book.quantity).toFixed(2)}</p>
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
                                ...
                                <div className="bg-[#F8FAFB] p-6 border-t border-[#E4E9E8]">
                                    <div className="flex justify-between items-center mb-6">
                                        <span className="font-bold text-[#848785]">Total Amount</span>
                                        <span className="font-black text-3xl text-[#101313]">${totalAmount.toFixed(2)}</span>
                                    </div>
                                    <Button
                                        onClick={handleCreateOrder}
                                        disabled={selectedBooks.length === 0 || createOrderMutation.isPending}
